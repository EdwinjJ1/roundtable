...
describe('loadAgentMemory', () => {
  it('returns an empty memory when nothing was written', async () => {
    const memory = await loadAgentMemory({ workspace, agentId: AGENT, ownerId: OWNER });
    expect(memory.facts).toEqual([]);
    expect(memory.plan).toBeNull();
    expect(memory.compactionNeeds).toEqual([]);
  });

  it('merges both scopes and lets the project fact win a slug collision', async () => {
    await writeFact(globalMemoryDir(OWNER, AGENT), 'css-style', fact('global version', 'global body'));
    await writeFact(globalMemoryDir(OWNER, AGENT), 'only-global', fact('only in global', 'kept'));
    await writeFact(projectMemoryDir(workspace, AGENT), 'css-style', fact('project version', 'project body'));

    const memory = await loadAgentMemory({ workspace, agentId: AGENT, ownerId: OWNER });
    const collided = memory.facts.find((item) => item.slug === 'css-style');
    expect(collided?.scope).toBe('project');
    expect(collided?.description).toBe('project version');
    expect(memory.facts.some((item) => item.slug === 'only-global')).toBe(true);
  });
...
describe('syncProjectMemoryToGlobal', () => {
  it('mirrors project facts into the global store and rebuilds the index', async () => {
    await writeFact(projectMemoryDir(workspace, AGENT), 'lens-ranking', fact('lens ranking model', 'body'));

    const result = await syncProjectMemoryToGlobal({ workspace, agentId: AGENT, ownerId: OWNER });
    expect(result.synced).toEqual(['lens-ranking']);
    expect(result.skipped).toEqual([]);

    const globalFact = await readFile(join(globalMemoryDir(OWNER, AGENT), 'lens-ranking.md'), 'utf8');
    expect(globalFact).toContain('body');
    const index = await readFile(join(globalMemoryDir(OWNER, AGENT), 'MEMORY.md'), 'utf8');
    expect(index).toContain('- [lens-ranking](lens-ranking.md) — lens ranking model');
  });

  it('updates existing slugs but skips NEW slugs when the global store is full', async () => {
    for (let i = 0; i < MEMORY_LIMITS.storeMaxFacts; i += 1) {
      await writeFact(globalMemoryDir(OWNER, AGENT), `old-${String(i).padStart(2, '0')}`, fact(`old ${i}`, 'old body'));
    }
    await writeFact(projectMemoryDir(workspace, AGENT), 'old-00', fact('old 0 updated', 'updated body'));
    await writeFact(projectMemoryDir(workspace, AGENT), 'brand-new', fact('new fact', 'new body'));

    const result = await syncProjectMemoryToGlobal({ workspace, agentId: AGENT, ownerId: OWNER });
    expect(result.synced).toContain('old-00');
    expect(result.skipped).toContain('brand-new');

    const updated = await readFile(join(globalMemoryDir(OWNER, AGENT), 'old-00.md'), 'utf8');
    expect(updated).toContain('updated body');
    const index = await readFile(join(globalMemoryDir(OWNER, AGENT), 'MEMORY.md'), 'utf8');
    expect(index).toContain('not synced: store at capacity');
  });

  it('keeps owners isolated: one user\'s sync never lands in another\'s store', async () => {
    await writeFact(projectMemoryDir(workspace, AGENT), 'secret', fact('secret fact', 'secret body'));
    await syncProjectMemoryToGlobal({ workspace, agentId: AGENT, ownerId: OWNER });

    const other = await loadAgentMemory({ workspace: join(tempDir, 'other-ws'), agentId: AGENT, ownerId: 'user-2' });
...
describe('concurrent writers (same agent, one scheduler wave)', () => {
  it('serializes writeProjectFact so capacity is never overshot and the index stays consistent', async () => {
    const { writeProjectFact, projectMemoryDir } = await import('../src/server/actions/agent-memory.js');
  ...
  it('serializes concurrent global syncs from two workspaces without corrupting the index', async () => {
    const otherWorkspace = join(tempDir, 'workspace-2');
    await mkdir(otherWorkspace, { recursive: true });
    await writeFact(projectMemoryDir(workspace, AGENT), 'from-ws-one', fact('from ws one', 'body one'));
    await writeFact(projectMemoryDir(otherWorkspace, AGENT), 'from-ws-two', fact('from ws two', 'body two'));

    await Promise.all([
      syncProjectMemoryToGlobal({ workspace, agentId: AGENT, ownerId: OWNER }),
      syncProjectMemoryToGlobal({ workspace: otherWorkspace, agentId: AGENT, ownerId: OWNER }),
    ]);

    const globalMemory = await loadAgentMemory({ workspace: null, agentId: AGENT, ownerId: OWNER });
    expect(globalMemory.facts.map((item) => item.slug).sort()).toEqual(['from-ws-one', 'from-ws-two']);
    const index = await readFile(join(globalMemoryDir(OWNER, AGENT), 'MEMORY.md'), 'utf8');
    expect(index).toContain('from-ws-one');
    expect(index).toContain('from-ws-two');
  });
});
